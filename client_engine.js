  ...
  window.fetch = async function(resource, init) {
    ...
    if (pathname === '/api/status/toggle' && method === 'POST') {
      ...
      if (dbStatus[type]) {
        ...
        reportActionToServer(`Baixa de item (${type})`, `ID: ${id} -> ${Boolean(done) ? 'CONCLUÍDO' : 'PENDENTE'}`);
        return jsonRes({
          success: true,
    ...
    if (pathname === '/api/status/batch-toggle' && method === 'POST') {
      ...
      if (dbStatus[type] && Array.isArray(ids)) {
        ...
        saveLocalStatus(dbStatus);
        reportActionToServer(`Baixa em lote (${type})`, `${ids.length} itens -> ${Boolean(done) ? 'CONCLUÍDO' : 'PENDENTE'}`);
        return jsonRes({
          success: true,
          count: ids.length,
          progress: getProgressStats()
